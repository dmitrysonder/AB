...
def get_charities(count=200, start=0, sort='richest', lang='eng', filter_value='',
                  search='', alphabet='', confirmed=True):
    payload = {'count': count, 'start': start, 'sort': sort, 'lang': lang, 'filter': filter_value, 'search': search,
...
def get_news(filter_value='', uuid='', count=200, start=2, lang='de'):
    payload = {'uuid': uuid, 'filter': filter_value, 'count': count, 'start': start, 'lang': lang}
...
def post_registration(email, password, firstname='APITESTER', lastname='DEFAULT', birthdate='1991-07-04', sex='1'):
    payload = {'email': email, 'password': password, 'firstname': firstname,
               'lastname': lastname, 'birthdate': birthdate, 'sex': sex}
...
def post_member_update(email, password, newemail='', newpassword='', firstname='', lastname='', birthdate='', sex='0'):
    payload = {'email': newemail, 'password': newpassword, 'firstname': firstname,
               'lastname': lastname, 'birthdate': birthdate, 'sex': sex}
    headers = {'Content-type': 'application/json'}
